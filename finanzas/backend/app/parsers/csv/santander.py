"""Parser for Santander Chile CSV export format.

Expected headers:
Fecha;Descripcion;Cargo;Abono;Saldo Disponible
"""
import csv
import io
...
class SantanderCSVParser(BaseParser):
    ...
    def can_parse(self, filename: str, file_content: bytes) -> bool:
        ...
        try:
            ...
            return _HEADER_KEYS.issubset(cols) and "SANTANDER" in text[:300].upper()
        except Exception:
            return False
    ...
    def parse(self, file: io.IOBase, filename: str) -> list[RawTransaction]:
        ...
        for row in reader:
            ...
            try:
                ...
                raise ParseError(f"Santander row error: {row} -> {e}")
