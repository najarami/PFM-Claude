"""PDF parser for Banco de Chile account statements."""
import io
import re
...
class BancoChilePDFParser(BasePDFParser):
    bank = BankName.BANCO_CHILE

    def _matches_bank(self, first_page_text: str) -> bool:
        return "BANCO DE CHILE" in first_page_text.upper()
    ...
    def parse(self, file: io.IOBase, filename: str) -> list[RawTransaction]:
        ...
        for line in lines:
            ...
            try:
                ...
                raise ParseError(f"BancoChile PDF line error: '{line}' -> {e}")
