"""PDF parser for Santander Chile account statements."""
import io
import re
...
class SantanderPDFParser(BasePDFParser):
    bank = BankName.SANTANDER

    def _matches_bank(self, first_page_text: str) -> bool:
        return "SANTANDER" in first_page_text.upper()
    ...
    def parse(self, file: io.IOBase, filename: str) -> list[RawTransaction]:
        ...
        for line in lines:
            ...
            try:
                ...
                raise ParseError(f"Santander PDF line error: '{line}' -> {e}")
